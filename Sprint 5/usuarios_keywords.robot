...
POST Endpoint /Usuarios
    &{payload}              Create Dictionary       nome=toninho     email=toninhozs@outlook.com     password=sadass   administrador=true
    ${response}             POST On Session     Serverest           /usuarios       data=&{payload}
    Log to Console          response ${response.content}
    Set Global Variable     ${response}

PUT Endpoint /usuarios
    &{payload}              Create Dictionary        nome=jevsosdasadn priecst      email=tedfvhdjahvdfkasjf@gmail.com      password=sadasfbsafclear    administrador=true
